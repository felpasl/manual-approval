...
func newCommentLoopChannel(ctx context.Context, apprv *approvalEnvironment, client *github.Client, pollingInterval time.Duration) chan commentLoopResult {
	...
	go func() {
		for {
			...
			approved, approver, err := approvalFromComments(comments, apprv.issueApprovers, apprv.minimumApprovals)
			if err != nil {
			...
			case approvalStatusApproved:
				...
				channel <- commentLoopResult{exitCode: 0, approver: approver, status: approvalStatusApproved}
			...
			case approvalStatusDenied:
				...
				channel <- commentLoopResult{exitCode: 1, approver: approver, status: approvalStatusDenied}
				close(channel)
			}
...
func main() {
	...
	case result := <-commentLoopChannel:
		...
		outputs := map[string]string {
			...
			"issue-approver": result.approver,
		}
