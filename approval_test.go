...
func TestApprovalFromComments(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			approvers:      []string{login1},
			expectedStatus: approvalStatusApproved,
			expectedApprover: login1u,
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			actualStatus, actualApprover, err := approvalFromComments(testCase.comments, testCase.approvers, testCase.minimumApprovals)
			if err != nil {
			...
			if actualApprover != testCase.expectedApprover {
				t.Fatalf("actual approver %s, expected %s", actualApprover, testCase.expectedApprover)
			}
		})
	}
}
